...
class Wiimote:
    def __init__(self, path, player_index, sensor_bar_above=True):
        ...
        self.up = False
        self.down = False
        self.sep = 0.0        # normalized spacing between bar dots (distance proxy)
        self._miss = 0
        self._smooth = None
    ...
    def _run(self):
        while not self._stop.is_set():
            ...
            if dots:
                ...
                self._miss = 0
                if len(dots) >= 2:
                    dx = (dots[0][0] - dots[1][0]) / IR_CAM_W
                    dy = (dots[0][1] - dots[1][1]) / IR_CAM_H
                    s = (dx * dx + dy * dy) ** 0.5
                    self.sep = self.sep + 0.3 * (s - self.sep)
            else:
                # hysteresis: only drop visibility after ~150 ms of misses
                self._miss += 1
                if self._miss > 15:
                    self.visible = False
